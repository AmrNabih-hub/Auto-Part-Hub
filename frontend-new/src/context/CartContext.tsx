...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addToCart = async (productId: string, quantity: number = 1) => {
    ...
    try {
      ...
      toast.error(axios.isAxiosError(err) ? err.response?.data?.message || 'Failed to add to cart.' : 'Failed to add to cart.');
    }
  };
  ...
  const removeFromCart = async (productId: string) => {
    ...
    try {
      ...
      toast.error(axios.isAxiosError(err) ? err.response?.data?.message || 'Failed to remove from cart.' : 'Failed to remove from cart.');
    }
  };
